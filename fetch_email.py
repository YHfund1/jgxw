...
def find_latest_email(mail):
    ...
    for msg_id in msg_ids[:10]:
        ...
        # 收集xlsx附件文件名（注意：附件文件名中的日期才是数据实际截至日期）
        attachments = []
        for part in msg.walk():
            filename = part.get_filename()
            if filename and filename.endswith('.xlsx') and not filename.startswith('~$'):
                attachments.append(decode_mime_str(filename))

        if not attachments:
            print(f'  邮件 [{subject}] 无xlsx附件，跳过')
            continue

        # 取主题日期后缀与附件数据日期中较新的一个作为邮件数据日期
        candidates = []
        subj_match = re.search(r'\u2014\u2014(\d{4})$', subject)
        if subj_match:
            candidates.append(subj_match.group(1))
        for fn in attachments:
            m = re.search(r'更新至(\d{8})', fn)
            if m:
                candidates.append(m.group(1)[4:])  # YYYYMMDD → MMDD
        if candidates:
            latest_mmdd = max(candidates)
            # 检查本地是否已有包含该日期的文件（含更新的日期）
            existing = glob.glob(os.path.join(DOWNLOAD_DIR, '*快照*.xlsx'))
            local_mmdds = []
            for f in existing:
                fm = re.search(r'更新至(\d{8})', os.path.basename(f))
                if fm:
                    local_mmdds.append(fm.group(1)[4:])
            if local_mmdds and max(local_mmdds) >= latest_mmdd:
                print(f'  邮件 [{subject}] 数据日期 {latest_mmdd}，本地已有更新或相同日期的源文件，跳过')
                continue
